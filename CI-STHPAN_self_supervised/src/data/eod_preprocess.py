...
class EOD_Preprocessor:
    ...
    def _transfer_EOD_str(self, selected_EOD_str, tra_date_index):
        ...
        for row, daily_EOD in enumerate(selected_EOD_str):
            ...
            daily_EOD_.append(tra_date_index[date_str])
            for col in range(1, selected_EOD_str.shape[1]):
                daily_EOD_.append(float(daily_EOD[col]))
            if daily_EOD_[-1] != 0:
                selected_EOD.append(daily_EOD_)
        selected_EOD = np.array(selected_EOD)
        return selected_EOD
    ...
    '''
    def generate_feature(self, selected_tickers_fname, begin_date,valid_index,opath,
        ...
        for stock_index, single_EOD in enumerate(self.data_EOD):
            ...
            for row in selected_EOD[:, 0]:
                row = int(row)
                if flag and row >= pad_begin:   # offset for the first 30-days average
                    begin_date_row = row
                    flag = False
                if row >= valid_index+pad_begin: # offset for the train set
                    end_train_row = row
                    break
            ...
            '''
            # print(selected_EOD[begin_date_row:end_train_row, 4].shape)
            pri_min = np.min(selected_EOD[begin_date_row:end_train_row, 4])
            price_max = np.max(selected_EOD[begin_date_row:end_train_row, 4])
            ...
            '''
                ...
                date_index, 5-day, 10-day, 20-day, 30-day, Close
            ...
            features = pd.DataFrame(features,columns=[['date','5-day', '10-day', '20-day', '30-day', 'Close']])
            features['date'] = trading_dates[pad_begin:]
            # write out
            features.to_csv(os.path.join(opath, self.market_name + '_' +
                                    self.tickers[stock_index] + '_' +
                                    str(return_days) + '.csv'),index=False)
        np.savetxt(os.path.join(opath, self.market_name + '_' + 'price_max' + '.csv'), 
                   price_max_list,fmt='%.6f', delimiter=',')
...
if __name__ == '__main__':
    ...
    if args.path is None:
        args.path = './datasets/stock/google_finance'
    ...
    if args.market == 'TSE':
        processor.generate_feature(
            ...
            datetime.strptime('2015-09-14', processor.date_format),693,
        ...
        processor.generate_feature(
            processor.market_name + '_tickers_qualify_dr-0.98_min-5_smooth.csv',
            datetime.strptime('2012-11-19 00:00:00', processor.date_format),756,
            os.path.join(processor.data_path, '..', '2013-01-01'), return_days=1,
            pad_begin=29
        )