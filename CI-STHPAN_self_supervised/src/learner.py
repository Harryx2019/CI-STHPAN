...
class Learner(GetAttr):
    ...
    def __init__(self, dls, model, graph, ci, rel_type, market, k,
                        loss_func=None, 
                        lr=1e-3, 
                        cbs=None, 
                        metrics=None, 
    ...
    def get_relation(self, flag):
        ...
        if self.graph:
            ...
            if self.rel_type == 0:
                if self.ci == 1:
                    if flag == 'train':
                        hyperedge_index = np.load(os.path.join('..','..','src/data/datasets/stock/relation','all',
                                                            self.market + '_all_relation_train_fine.npy'))
                    else:
                        hyperedge_index = np.load(os.path.join('..','..','src/data/datasets/stock/relation','all',
                                                            self.market + '_all_relation_valid_fine.npy'))
                else:
                    if flag == 'train':
                        hyperedge_index = np.load(os.path.join('..','..','src/data/datasets/stock/relation','all',
                                                            self.market + '_all_relation_train_mix.npy'))
                    else:
                        hyperedge_index = np.load(os.path.join('..','..','src/data/datasets/stock/relation','all',
                                                            self.market + '_all_relation_valid_mix.npy'))
            # indurtry relation
            elif self.rel_type == 1:
                hyperedge_index = np.load(os.path.join('..','..','src/data/datasets/stock/relation','industry',
                                                    self.market + '_industry_relation.npy'))
                # channel independent
                if self.ci == 1:
                    num_stocks = hyperedge_index.shape[0]
                    num_edges = hyperedge_index.shape[1]
                    industry_relation = np.zeros([self.dls.vars,num_stocks,num_edges],dtype=int)
                    for m in range(self.dls.vars):
                        industry_relation[m] = hyperedge_index
                    hyperedge_index = industry_relation
            # wiki relation
            elif self.rel_type == 2:
                hyperedge_index = np.load(os.path.join('..','..','src/data/datasets/stock/relation','wikidata',
                                                    self.market + '_wiki_relation.npy'))
                # channel independent
                if self.ci == 1:
                    num_stocks = hyperedge_index.shape[0]
                    num_edges = hyperedge_index.shape[1]
                    wiki_relation = np.zeros([self.dls.vars,num_stocks,num_edges],dtype=int)
                    for m in range(self.dls.vars):
                        wiki_relation[m] = hyperedge_index
                    hyperedge_index = wiki_relation
            # dtw relation
            elif self.rel_type == 3:
                if self.ci == 1:
                    if flag == 'train':
                        hyperedge_index = np.load(os.path.join('..','..','src/data/datasets/stock/relation','dtw',
                                                            self.market + '_dtw_train_fine_top'+str(self.k)+'.npy'))
                    else:
                        hyperedge_index = np.load(os.path.join('..','..','src/data/datasets/stock/relation','dtw',
                                                            self.market + '_dtw_valid_fine_top'+str(self.k)+'.npy'))
                else:
                    if flag == 'train':
                        hyperedge_index = np.load(os.path.join('..','..','src/data/datasets/stock/relation','dtw',
                                                            self.market + '_dtw_relation_train_top'+str(self.k)+'_mix.npy'))
                    else:
                        hyperedge_index = np.load(os.path.join('..','..','src/data/datasets/stock/relation','dtw',
                                                            self.market + '_dtw_relation_valid_top'+str(self.k)+'_mix.npy'))
            # industry_wiki relation
            elif self.rel_type == 4:
                hyperedge_index = np.load(os.path.join('..', '..', 'src/data/datasets/stock/relation/industry_wiki',
                                                    self.market + '_industry_wiki_relation.npy'))
                # channel independent
                if self.ci == 1:
                    num_stocks = hyperedge_index.shape[0]
                    num_edges = hyperedge_index.shape[1]
                    industry_wiki_relation = np.zeros([self.dls.vars,num_stocks,num_edges],dtype=int)
                    for m in range(self.dls.vars):
                        industry_wiki_relation[m] = hyperedge_index
                    hyperedge_index = industry_wiki_relation
            # industry_dtw relation
            elif self.rel_type == 5:
                # channel independent
                if self.ci == 1:
                    if flag == 'train':
                        hyperedge_index = np.load(os.path.join('..', '..', 'src/data/datasets/stock/relation/industry_dtw',
                                                            self.market + '_industry_dtw_relation_train_fine.npy'))
                    else:
                        hyperedge_index = np.load(os.path.join('..', '..', 'src/data/datasets/stock/relation/industry_dtw',
                                                            self.market + '_industry_dtw_relation_valid_fine.npy'))
                # channel mixing
                else:
                    if flag == 'train':
                        hyperedge_index = np.load(os.path.join('..', '..', 'src/data/datasets/stock/relation/industry_dtw',
                                                            self.market + '_industry_dtw_relation_train_mix.npy'))
                    else:
                        hyperedge_index = np.load(os.path.join('..', '..', 'src/data/datasets/stock/relation/industry_dtw',
                                                            self.market + '_industry_dtw_relation_valid_mix.npy'))
            # wiki_dtw relation
            elif self.rel_type == 6:
                # channel independent
                if self.ci == 1:
                    if flag == 'train':
                        hyperedge_index = np.load(os.path.join('..', '..', 'src/data/datasets/stock/relation/wiki_dtw',
                                                            self.market + '_wiki_dtw_relation_train_fine.npy'))
                    else:
                        hyperedge_index = np.load(os.path.join('..', '..', 'src/data/datasets/stock/relation/wiki_dtw',
                                                            self.market + '_wiki_dtw_relation_valid_fine.npy'))
                # channel mixing
                else:
                    if flag == 'train':
                        hyperedge_index = np.load(os.path.join('..', '..', 'src/data/datasets/stock/relation/wiki_dtw',
                                                            self.market + '_wiki_dtw_relation_train_mix.npy'))
                    else:
                        hyperedge_index = np.load(os.path.join('..', '..', 'src/data/datasets/stock/relation/wiki_dtw',
    ...
    def train_step(self, batch):
        ...
        self.xb = xb[0]
        self.yb = yb[0]
        self.mask_batch = mask_batch[0]
        self.price_batch = price_batch[0]
        self.gt_batch = gt_batch[0]
        ...
        if self.head_type == 'pretrain':
            ...
            f_dim = -1
            pred = pred[:, :, f_dim:]
            loss, reg_loss, rank_loss, rr = self.loss_func(pred.reshape((self.batch_size,1)), 
                                                           self.price_batch, self.gt_batch, self.mask_batch,
                                                           self.alpha, self.batch_size, self.device)
        return pred, loss

    def model_forward(self):
        self('before_forward')
        self.pred = self.model(self.xb, self.hyperedge_index, self.device)
        self('after_forward')
        return self.pred
    ...
    def _do_batch_test(self):   
        self.result, self.pred, self.true, self.mask = self.test_step(self.batch)     
    ...
    def test_step(self, batch):
        ...
        pred = torch.div((result - self.price_batch), self.price_batch)
        return result, pred, self.gt_batch, self.mask_batch
    ...
    def test(self, dl, market=None, weight_path=None, evaluate=None):
        """_summary_
        Args:
            test_data can be a tensor, numpy array, dataset or dataloader
        Returns:
        ...
        if weight_path is not None: self.load(weight_path)
        if market == 'NASDAQ':
            cb = GetTestCB(tickers_num=1026, test_dates = len(self.dl))
        elif market == 'NYSE':
        ...
        self('before_test')
        if self.graph:
            self.get_relation('test')
        self.model.eval()
        with torch.no_grad(): self.all_batches('test')
        self('after_test')
        self.results, self.preds, self.trues, self.masks = cb.results, cb.preds, cb.trues, cb.masks
        # calculate preformance
        if evaluate: 
            perf= evaluate(self.preds, self.trues, self.masks)
            return self.results, self.preds, self.trues, perf
        else: return self.results, self.preds, self.trues
